...
if __name__ == '__main__':
    if len(sys.argv) != 3: sys.exit("use save_stereo.py rosbag_file config_file")

    with open("save_stereo_config.yaml") as config_file:
        config = yaml.safe_load(config_file)
        assert config is not None, "Error opening configuration file"
    # for progress bar
    widgets = [' [',
    ...
    if not os.path.exists(left_image_path): os.makedirs(left_image_path)
    if not os.path.exists(right_image_path): os.makedirs(right_image_path)
    ...
    left_camera = 0
    right_camera = 1
    # Checking image synchrinicity and saving images.
