...
// static parameter
//loat lengths[] = {5,10,50,100,150,200,250,300,350,400};
float lengths[] = {100, 200, 300, 400, 500, 600, 700, 800, 1000, 1500, 2000, 2500, 3000, 3500, 4000};
...
struct errors
{
  ...
  float len;
  float speed;
  errors(int32_t first_frame, float r_err, float t_err, float len, float speed) : first_frame(first_frame), r_err(r_err), t_err(t_err), len(len), speed(speed) {}
};
...
vector<errors> calcDrift(vector<Matrix> &poses_gt, vector<Matrix> &poses_result)
{
...
void saveDriftPlots(vector<errors> &seq_err, string plot_error_dir, char *prefix)
{

  // file names
  char file_name_tl[1024];
  sprintf(file_name_tl, "%s/%s_drift_tl.txt", plot_error_dir.c_str(), prefix);
  char file_name_rl[1024];
  sprintf(file_name_rl, "%s/%s_drft_rl.txt", plot_error_dir.c_str(), prefix);
...
void saveStats(vector<errors> err, string dir, string name = "", Mail* mail=nullptr)
{
  float t_err = 0;
  float r_err = 0;
...
void saveSeqStats(vector<errors> err, string dir){
  float t_err = 0;
  float r_err = 0;
  // for all errors do => compute sum of t_err, r_err
  for (vector<errors>::iterator it = err.begin(); it != err.end(); it++)
...
bool eval(string result_sha, Mail* mail, int num_avg)
{
  ...
  for (int32_t i = 00; i < 11; i++)
  {
    ...
    vector<vector<Matrix>> pose_result_all;
    ...
    for (int i = 1; i <= num_avg; i++)
    {
      ...
      pose_result_all.push_back(loadPoses(result_dir + "/" + to_string(i) + "/" + file_name));
    }
    // check for errors
    for (int i = 0; i < pose_result_all.size(); i++)
      if (poses_gt.size() == 0 || pose_result_all[i].size() != poses_gt.size())
      {
        mail->msg("ERROR: Couldn't read (all) poses of: %s, %d", file_name, pose_result_all[i].size());
        return false;
      }

    //calculating avg of poses
    vector<Matrix> pose_result;
    for (int i = 0; i < pose_result_all[0].size(); i++)
    {
      Matrix m = Matrix(4, 4);
      m.zero();
      pose_result.push_back(m);
    }
    for (int i = 0; i < pose_result_all.size(); i++)
      for (int j = 0; j < pose_result.size(); j++)
        pose_result[j] = pose_result[j] + pose_result_all[i][j];

    for (int i = 0; i < pose_result.size(); i++)
    {
      pose_result[i] = pose_result[i] / pose_result_all.size();
    }

    // plot status
    mail->msg("Processing: %s, poses: %d/%d", file_name, pose_result.size(), poses_gt.size());

    // compute sequence errors
    vector<errors> seq_err = calcSequenceErrors(poses_gt, pose_result);
    mail->msg(file_name);
    //vector<errors> drift_err = calcDrift(poses_gt, pose_result);
    saveSequenceErrors(seq_err, error_dir + "/" + file_name);

    saveSeqStats(seq_err, error_dir + "/" + std::to_string(i));

    //saveSequenceErrors(drift_err, error_dir + "/drift_" + file_name);
    ...
    if (i <= 15)
    {
      ...
      savePathPlot(poses_gt, pose_result, plot_path_dir + "/" + file_name);
      vector<int32_t> roi = computeRoi(poses_gt, pose_result);
      ...
      //saveDriftPlots(drift_err, plot_error_dir, prefix);
  ...
  if (total_err.size() > 0)
  {
    ...
    saveStats(total_err, result_dir,"", mail);
  }
...
int32_t main(int32_t argc, char *argv[])
{
  ...
  if (argc != 3)
  {
    cout << "Usage: ./eval_odometry result_sha num_avg" << endl;
    return 1;
  }

  // read arguments
  string result_sha = argv[1];
  int num_avg = stoi(argv[2]);
  ...
  // run evaluation
  bool success = eval(result_sha, mail, num_avg);
