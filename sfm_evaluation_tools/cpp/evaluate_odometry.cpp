...
void plotPathPlot (string dir,vector<int32_t> &roi,int32_t idx) {
  ...
  for (int32_t i=0; i<2; i++) {
    ...
    if (i==0) {
      ...
      fprintf(fp,"set output \"seq.png\"\n");
    } else {
      fprintf(fp,"set term postscript eps enhanced color\n");
      fprintf(fp,"set output \"seq.eps\"\n");
    }
    ...
    fprintf(fp,"plot \"%02d.txt\" using 1:2 lc rgb \"#FF0000\" title 'Ground Truth' w lines,",0);
    fprintf(fp,"\"%02d.txt\" using 3:4 lc rgb \"#0000FF\" title 'Visual Odometry' w lines,",0);
...
bool eval (string result_sha,Mail* mail, string file_name) {
  ...
  vector<errors> total_err;
    ...
    // for first half => plot trajectory and compute individual stats
      ...
      savePathPlot(poses_gt,poses_result,plot_path_dir + "/00.txt");
      vector<int32_t> roi = computeRoi(poses_gt,poses_result);
      plotPathPlot(plot_path_dir,roi,0);
...
int32_t main (int32_t argc,char *argv[]) {
  ...
  if (argc!=3 ) {
    cout << "Usage: ./eval_odometry result_sha sequence_file_name" << endl;
    return 1;
  }
  ...
  Mail *mail;
  if (argc==5) mail = new Mail(argv[3]);
  else         mail = new Mail();
  mail->msg("Thank you for participating in our evaluation!");

  // run evaluation
  bool success = eval(result_sha,mail, file_name);
